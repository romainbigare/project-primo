...
function getSideEffects2(name, filename) {
...
function componentsResolver(name, { ssr }) {
  ...
  return {
    ...
    from: getSideEffects2(resolveId, `index.${ssr ? "umd" : "es"}.js`)
  };
}
...
function getSideEffects3(dirName, options) {
...
function resolveDirective(name, options) {
  ...
  if (compare(version, "1.1.0-beta.1", ">=") || nightly) {
    return {
      ...
      from: `${nightly ? "@element-plus/nightly" : "element-plus"}/${ssr ? "lib" : "es"}`,
      sideEffects: getSideEffects3(directive.styleName, options)
    };
  }
}
...
function getSideEffects5(version, path, importStyle, importStyleTheme) {
  if (!importStyle)
    return;
...
function getSideEffects6(importName, options) {
  const { importStyle = "css" } = options;
  if (!importStyle)
    return void 0;
...
function resolveComponent2(importName, options) {
  ...
  return name ? { name, from: libName, sideEffects: getSideEffects6(name, options) } : void 0;
}
    ...
    resolve: (name) => {
      if (name.startsWith("Van")) {
        ...
        return {
          ...
          sideEffects: getSideEffects7(kebabCase(partialName), options)
        };
      }
    }
  };
    ...
    resolve: (name) => {
      if (name.match(/^Veui[A-Z]/)) {
        ...
        if (!components3.has(componentName))
          return;
        const sideEffects = getSideEffects8(componentName, options);
...
function getSideEffects9(componentName) {
  const sideEffects = [
...
function getCompDir(compName) {
  let compPath;
  const total = matchComponents4.length;
  for (let i = 0; i < total; i++) {
    ...
    resolve: (name) => {
      if (name.match(/^I[A-Z]/)) {
        ...
        return {
          ...
          sideEffects: getSideEffects9(compName)
        };
      }
    }
  };
  ...
  return {
    ...
    resolve: (name) => {
      if (!directives) {
        let indexesJson;
        try {
          const corePath = resolveModule4("@vueuse/core") || process2.cwd();
          const path = resolveModule4("@vueuse/core/indexes.json") || resolveModule4("@vueuse/metadata/index.json") || resolveModule4("@vueuse/metadata/index.json", { paths: [corePath] });
